...
const STANCE_LABEL: Record<string, { text: string; tone: 'green' | 'amber' | 'red' | 'neutral' }> = {
  ...
  SETUJU: { text: 'Anda menyetujui', tone: 'neutral' },
  TOLAK: { text: 'Anda menyanggah', tone: 'blue' as 'neutral' },
};
...
export default async function ProdusenKomplain() {
  ...
  const riwayat = complaints.filter((c) => c.status !== 'MENUNGGU_SANGGAHAN');
            ...
            {perluTanggapan.map((c) => (
              <Card key={c.id} className="border-amber-200">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  {c.responseDeadline && (
                    <Badge tone="amber">{sisaWaktu(c.responseDeadline)}</Badge>
                  )}
                </div>
                ...
                <p className="text-xs text-ink/50">
                  Item: {c.order.items.map((i) => i.product.name).join(', ')}
                </p>
                ...
                <div className="mt-3">
                  <ComplaintRespondForm complaintId={c.id} />
                </div>
              </Card>
            ))}
          </div>
            ...
            {riwayat.map((c) => {
              ...
              return (
                <Card key={c.id}>
                  ...
                  <p className="mt-1 text-sm text-ink/70">{c.reason}</p>
                  {c.producerResponse && (
